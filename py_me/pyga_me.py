...
import __pyga_me_modules.events
import __pyga_me_modules.objects
...
class pyga_me:
    ...
    K_notes = k_notes
    Animation = animation
    events = __pyga_me_modules.events
    objects = __pyga_me_modules.objects
    V_NONE = k_notes.NONE
    V_DO = k_notes.V_DO
    V_RE = k_notes.V_RE
    V_MI = k_notes.V_MI
        ...
        if not os_me.path.FileHunter_TrueOrFalse(caminho):
            try:
                ...
                if elapsed > TimeLimitAllowed:
                    pyga_me.events.events_inmoment.append(pyga_me.events.event(pyga_me.LAGDOWNLOAD, elapsed))
                else:
                    pyga_me.events.events_inmoment.append(pyga_me.events.event(pyga_me.SAFEDOWNLOAD, elapsed))
            except Exception as e:
                pyga_me.events.events_inmoment.append(pyga_me.events.event(pyga_me.EXCEPTDOWNLOAD, [url, str(e)]))
        return caminho